"""CRUD routes for platform-global dictionaries and hierarchical dictionary items."""
...
from app.sys.dict.api.deps import require_any_workspace_member, require_super_admin
...
async def list_dicts(
    ...
    _user: User = Depends(require_any_workspace_member),
    ...
    if raw_code is not None and rows:
        flat = await svc.list_items_by_dict_code(session, dict_code=raw_code)
...
async def patch_dict(
    ...
    if not patch:
        row = await svc.get_dict(session, dict_id=dict_id)
        return _dict_to_list_out(row)
...
async def remove_dict(
    ...
    await svc.delete_dict(session, dict_id=dict_id)
...
async def list_dict_items(
    ...
    rows = await svc.list_items(session, dict_id=dict_id)

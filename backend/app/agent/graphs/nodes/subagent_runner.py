        ...
        async for event in subagent.astream_events(inputs, config=config_sub, version="v2"):
            ...
            if event.get("event") == "on_chain_end":
                ...
                if isinstance(out, dict) and out.get("messages"):
                    last_chain_messages = out["messages"]
                    output = extract_last_ai_text(out["messages"])

        if not output and last_chain_messages:
            output = extract_last_ai_text(last_chain_messages)

        if not output and settings.agent_subagent_ainvoke_fallback:
            log.warning(
                "subagent ainvoke fallback run_id={} step_id={}",
                deps.run_id,
                step.id,
                event="agent.subagent.ainvoke_fallback",

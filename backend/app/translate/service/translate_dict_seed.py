"""Ensure workspace ``TRANSLATE_STATUS`` / ``TRANSLATE_SEGMENT_STATUS`` dictionary rows exist."""
...
async def _ensure_dict_with_items(
    ...
    *,
    ...
    row = await dict_repo.get_dict_by_code_for_workspace(
...
async def ensure_translate_status_dicts(
    session: AsyncSession, *, workspace_id: uuid.UUID
) -> None:
    """Seed translate job/segment status dictionaries for one workspace if absent."""

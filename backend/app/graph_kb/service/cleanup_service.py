...
def _send_cleanup_task(*, workspace_id: uuid.UUID, graph_id: uuid.UUID, engine: str) -> None:
...
async def enqueue_cleanup(
    *,
    ...
    _ = user_id
    try:
        _send_cleanup_task(workspace_id=workspace_id, graph_id=graph_id, engine=engine)
    except Exception:

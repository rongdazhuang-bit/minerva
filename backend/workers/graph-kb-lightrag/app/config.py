"""LightRAG worker settings from ``.env.<WORKER_ENV>`` in the worker package root."""
...
def _env_file_paths() -> tuple[str, ...] | None:
    """Load only ``backend/workers/graph-kb-lightrag/.env.<WORKER_ENV>`` when present."""
...
class Settings(BaseSettings):
    """Typed configuration for the LightRAG GraphKB worker process."""
    ...
    graph_kb_embedding_dim: int = Field(
        default=1536,
        ge=1,
        validation_alias=AliasChoices(
            "GRAPH_KB_EMBEDDING_DIM",
            "graph_kb_embedding_dim",
        ),
    )
    ...
    def require_api_key_and_models(self) -> Self:
        ...
        if not self.graph_kb_lightrag_worker_api_key.strip():
            raise ValueError(
                "GRAPH_KB_LIGHTRAG_WORKER_API_KEY is required "
...
def load_settings() -> Settings:
    ...
    try:
        ...
        print(f"[error] LightRAG worker config: {exc}", file=sys.stderr)
        sys.exit(1)

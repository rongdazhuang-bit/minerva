...
- 其它 `app/*` 模块仅依赖 `llm` 的 **稳定服务接口与 DTO**，不直接依赖具体 SDK 类型泄漏（策略实现细节封装在 `strategies/` 内）。
...
- **错误模型**：对外（HTTP）与对内（服务异常）统一为项目既有异常体系或 `llm` 内轻量错误类型，**不向上游原文泄露密钥**。
...
- 路由可挂载在现有 `app/api/router.py` 下前缀（例如 `/workspaces/{workspace_id}/llm/...`），**鉴权与权限与项目内部 API 一致**（若当前内部接口无统一鉴权，则与实现计划同步选用最小约束）。

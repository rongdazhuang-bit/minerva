...
- **字典约束**：
  ...
  - `provider_name`、`model_type` 均落库存字典项 **code**（key）；列表/分组展示时按 code 反查 name。
...
- `provider_name`、`model_type` 落库存字典项 `code`（key）；展示层按字典反查 name。
...
- `provider_name`：必填，必须命中字典 `MODEL_PROVIDER.code`（key）。
